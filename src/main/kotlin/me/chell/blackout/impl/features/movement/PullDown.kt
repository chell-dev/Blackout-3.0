...
class PullDown: ToggleFeature("PullDown", Category.Movement, false) {
    ...
    fun onPlayerTick(event: PlayerTickEvent) {
        ...
        val ray = mc.world!!.raycast(RaycastContext(player.pos, player.pos.add(0.0, -maxHeight.value, 0.0), RaycastContext.ShapeType.OUTLINE, RaycastContext.FluidHandling.ANY, player))
        val distance = player.pos.y - ray.pos.y
        if(distance >= minHeight.value) {
            player.upwardSpeed = -10f
        }
    }
