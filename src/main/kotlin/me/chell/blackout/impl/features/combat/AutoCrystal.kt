...
class AutoCrystal : Feature("AutoCrystal", Category.Combat) {
    ...
    private val noGappleSwitch = register(
        Setting(
            "Don't Switch While Gappling",
            true
        ) { page.value == Page.Place && autoSwitch.value != SwitchMode.Off })
    ...
    private val facePlaceBind =
        register(Setting("FacePlace Bind", Bind.Toggle(onEnable = {}, onDisable = {})) { page.value == Page.Place })
    ...
    private fun hit(): Boolean {
        ...
        var crystal = if (renderPos != null) world.getEntitiesByClass(
            EndCrystalEntity::class.java,
            Box(renderPos!!.up())
        ) { true }.firstOrNull() else null
        if (crystal == null) crystal =
            player.getClosestEntity(EndCrystalEntity::class.java, hitRange.value) as EndCrystalEntity? ?: return false
    ...
    private fun place() {
        ...
        val target = player.getClosestEntity(
            PlayerEntity::class.java,
            playerRange.value
        ) { it != player && !player.isFriend() } as PlayerEntity?
        ...
        for (block in list) {
            ...
            val ray = world.raycast(
                RaycastContext(
                    player.eyePos,
                    block.toCenterPos().add(0.0, 0.5, 0.0),
                    ShapeType.COLLIDER,
        ...
        interactionManager.interactBlock(
            player,
            hand,
            raytrace ?: BlockHitResult(placePos.toCenterPos().add(0.0, 0.5, 0.0), Direction.UP, placePos, false)
        )
    ...
    private fun ClientPlayerEntity.getClosestEntity(
    ...
    private fun BlockPos.canPlaceCrystal(): Boolean {
        ...
        return !world.getOtherEntities(
            null,
            Box(up.x.toDouble(), up.y.toDouble(), up.z.toDouble(), up.x + 1.0, up.y + 2.0, up.z + 1.0)
        ).isNotEmpty() // todo up.y + 1.0 ?\
    ...
    private fun BlockPos.getExplosionDamage(target: LivingEntity): Float {
        ...
        damage = DamageUtil.getDamageLeft(
            ...
            target.getAttributeValue(EntityAttributes.GENERIC_ARMOR_TOUGHNESS).toFloat()
        )

        if (target.hasStatusEffect(StatusEffects.RESISTANCE)) {
            damage = max(
                (damage * (25 - ((target.getStatusEffect(StatusEffects.RESISTANCE)!!.amplifier + 1) * 5)).toFloat()) / 25.0f,
                0.0f
            )
        }
