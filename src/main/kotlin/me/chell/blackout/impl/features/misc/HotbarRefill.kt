...
class HotbarRefill : ToggleFeature("Hotbar Refill", Category.Misc, false) {
    ...
    private val whitelist =
        register(Setting("Whitelist", mutableListOf(Items.EXPERIENCE_BOTTLE, Items.ENCHANTED_GOLDEN_APPLE)))
        ...
        for (i in 0 until 9) {
            ...
            if (stack.count < stack.maxCount && whitelist.value.contains(stack.item)) {
                for (slot in 9 until player.inventory.main.size) {
                    if (player.inventory.getStack(slot).item == stack.item && player.inventory.getStack(slot).nbt?.equals(
                            stack.nbt
                        ) == true
                    ) {
