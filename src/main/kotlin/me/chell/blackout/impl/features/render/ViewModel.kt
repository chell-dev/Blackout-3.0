...
class ViewModel: ToggleFeature("ViewModel", Category.Render, false) {
    ...
    private val liScale = register(Setting("Left Scale", 1.0f, 0.1f, 2f) { page.value == Page.LeftItem })
    ...
    private val liRotX = register(Setting("Left X Rotation", 0.0f, -180f, 180f) { page.value == Page.LeftItem })
    private val liRotY = register(Setting("Left Y Rotation", 0.0f, -180f, 180f) { page.value == Page.LeftItem })
    private val liRotZ = register(Setting("Left Z Rotation", 0.0f, -180f, 180f) { page.value == Page.LeftItem })
    private val liEquip = register(Setting("Left Equip Progress", false) { page.value == Page.LeftItem })

    private val riScale = register(Setting("Right Scale", 1.0f, 0.1f, 2f) { page.value == Page.RightItem })
    ...
    private val riRotX = register(Setting("Right X Rotation", 0.0f, -180f, 180f) { page.value == Page.RightItem })
    private val riRotY = register(Setting("Right Y Rotation", 0.0f, -180f, 180f) { page.value == Page.RightItem })
    private val riRotZ = register(Setting("Right Z Rotation", 0.0f, -180f, 180f) { page.value == Page.RightItem })
