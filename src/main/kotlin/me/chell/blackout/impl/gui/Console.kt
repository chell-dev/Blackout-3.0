...
object Console : DrawableHelper() {
    ...
    fun render(matrices: MatrixStack, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        if (y + (if (isOpen) height else titleHeight) > mc.window.scaledHeight) y =
            mc.window.scaledHeight - (if (isOpen) height else titleHeight)

        fill(matrices, x + border, y, x + width - border, y + titleHeight, borderColor.rgb)
        textRenderer.draw(
            matrices,
            Formatting.GREEN + "Console",
            x + border + padding.toFloat(),
            y + (titleHeight / 2f) - (textRenderer.fontHeight / 2f),
            -1
        )
        ...
        // lines
        var lineY =
            y + height - border - inputHeight - padding.toFloat() - (lines.size * textRenderer.fontHeight) + scroll
        ...
        textRenderer.draw(
            ...
            renderInput,
            x + border + padding.toFloat(),
            y + height - (inputHeight / 2f) - (textRenderer.fontHeight / 2f),
            -1
        )
    }
    ...
    fun mouseScrolled(mouseX: Double, mouseY: Double, amount: Double): Boolean {
        if (isOpen && mouseX >= x && mouseY >= y && mouseX <= x + width && mouseY <= y + height) {
            ...
            scroll = min(
                max(0, a),
                max(
                    (lines.size * textRenderer.fontHeight) - (height - titleHeight - border - inputHeight - padding - padding),
                    0
                )
            )
            return true
